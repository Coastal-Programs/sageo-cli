...
func TestEnvOverridesAndRedaction(t *testing.T) {
	...
	configPath := filepath.Join(t.TempDir(), "sageo-config.json")
	t.Setenv("SAGEO_CONFIG", configPath)
	t.Setenv("SAGEO_SERP_PROVIDER", "serpapi")
	t.Setenv("SAGEO_SERP_API_KEY", "serp-override-key")
	t.Setenv("SAGEO_APPROVAL_THRESHOLD_USD", "5.25")
	t.Setenv("SAGEO_GSC_PROPERTY", "sc-domain:example.com")
	t.Setenv("SAGEO_GSC_CLIENT_ID", "override-client-id")
	t.Setenv("SAGEO_GSC_CLIENT_SECRET", "override-client-secret")
