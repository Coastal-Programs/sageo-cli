...
import (
	...
	"github.com/jakeschepis/sageo-cli/internal/backlinks"
...
(populated by 'sageo labs competitors').`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if target == "" {
				return output.PrintCodedError(output.ErrBacklinksFailed, "target is required",
			...
			if competitors != "" {
				...
				if !state.Exists(".") {
					return output.PrintCodedError(output.ErrBacklinksFailed, "no competitors provided and no state.json found",
						fmt.Errorf("use --competitors or run 'sageo labs competitors' first"), nil, output.Format(*format))
				}
